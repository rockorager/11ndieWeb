...
exports.handler = async (event, context) => {
    ...
    // TODO add optional domain restrictino to wmverifier call
    const res = await wmverifier(data.source, data.target);
    ...
    for (let i in webmentions.content.children) {
        if (
            webmentions.content.children[i]["wm-source"] === data.source &&
            webmentions.content.children[i]["wm-target"] === data.target
        ) {
            comboIndex = i;
        }
    }
